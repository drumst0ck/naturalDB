  ...
  return (
      ...
      >
        <TableCell className="text-left py-4">
          {isConnected ? (
              <Link href={`/db/${db.id}`} className="flex items-center space-x-2 text-blue-400 hover:text-blue-300">
                <DatabaseName />
              </Link>
          ) : (
        ...
        <TableCell className="text-center py-4">
          <span className="px-2 py-1 bg-gray-700 rounded-full text-sm">{db.type}</span>
        </TableCell>
        <TableCell className="text-center py-4">
          <div className="flex justify-center">
            <StatusIndicator isConnected={isConnected} isLoading={isLoading} isError={isError} />
          </div>
        </TableCell>
...
const DbTable = () => {
  ...
  return (
      <div className="bg-gray-900 p-6 rounded-3xl shadow-lg max-w-4xl mx-auto">
        <Table>
          <TableHeader>
            <TableRow className="border-b border-gray-700">
              <TableHead className="text-left text-gray-400 py-4">Database</TableHead>
              <TableHead className="text-center text-gray-400 py-4">Type</TableHead>
              <TableHead className="text-center text-gray-400 py-4">Status</TableHead>
              <TableHead className="text-right text-gray-400 py-4">Actions</TableHead>
            </TableRow>
          </TableHeader>
          <TableBody>
            <AnimatePresence>
