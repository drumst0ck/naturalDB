...
export const clearConsole = (id, dbSchema) => {
...
export const initializeChat = async (db, setDbSchema, id) => {
  try {
    ...
    if (savedMessages.length > 0) {
      ...
      const initialAIMessage = addTimestamp({
        id: "initial-message",
        role: "assistant",
        content: schema, // Store the raw schema in the content
        schema: schema, // Add a separate schema property
      });
